...
for i in range(1,13):
    ...
    for year in range(2011,2021):
        if i == 1:
              ...
              w1 = ["wind/"+x for x in l("wind/") if f"MERRA2_400.tavg1_2d_slv_Nx.{year}{month}" in x] #WIND DATA FILES
...
for i in range(0,12):
      ...
      for file in m[i]:
          ...
          dayAvgs.append(np.mean(PM25, axis=0))#APPENDING DAYS AND TAKING MEAN

      dayAvgs = np.array(dayAvgs)
      moAvgs = np.mean(dayAvgs,axis=0)
      moAvgs = moAvgs*(10**9)#FOR UNIT CHANGING
      #print(moAvgs)
...
#PLOTTING WIND 
      ...
      plt.title('PM2.5 AVERAGE: '+ MAH[i] +' (2011-2020)' )
      plt.show()
